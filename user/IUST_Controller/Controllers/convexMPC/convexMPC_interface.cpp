...
void update_problem_data(double* p, double* v, double* q, double* w, double* r, double yaw, double* weights, double* state_trajectory, double alpha, int* gait, bool iust)
{
  ...
  solve_mpc(&update, &problem_configuration, iust);
  has_solved = 1;
}
...
void update_problem_data_floats(float* p, float* v, float* q, float* w,
                                ...
                                float* state_trajectory, float alpha, int* gait, bool iust)
{
  update.alpha = alpha;
  update.yaw = yaw;
  mint_to_u8(update.gait,gait,4*problem_configuration.horizon);
